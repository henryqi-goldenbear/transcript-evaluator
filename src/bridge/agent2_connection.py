...
class Agent2Bridge:
    def __init__(self, agent2_script: Path | None = None) -> None:
        self.agent2_script = agent2_script
    ...
    def start_agent2(self, ws_url: str) -> None:
        ...
        if self.agent2_script is not None:
            command = [sys.executable, str(self.agent2_script.resolve()), ws_url]
        else:
            command = [sys.executable, "-m", "src.agent2.agent2", ws_url]
        if os.environ.get("MISTRAL_AGENT2_DRY_RUN", "").lower() in {"1", "true", "yes"}:
            command.append("--dry-run")
        subprocess_log = Path("logs") / "agent2" / "system" / "agent2_subprocess.log"
        subprocess_log.parent.mkdir(parents=True, exist_ok=True)
        log_handle = subprocess_log.open("a", encoding="utf-8")
        log_handle.write(f"{local_time_mark()} Starting Agent 2: {' '.join(command)}\n")
        log_handle.flush()
        subprocess.Popen(
            command,
            cwd=Path.cwd(),
