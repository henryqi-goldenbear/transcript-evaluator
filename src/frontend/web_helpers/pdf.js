(function() {
  ...
  function getDimensionOrderForRubric(parsed) {
    return ["clarity", "relevance", "specificity", "self_awareness"];
  }

  function ratingLabel(row) {
    const raw = row?.rating ?? row?.score;
    if (typeof raw === "number") {
      return ["very poor", "poor", "average", "good", "excellent"][
        Math.max(1, Math.min(5, Math.round(raw))) - 1
      ];
    }
    const rating = String(raw || "").trim().toLowerCase().replace(/[_-]+/g, " ");
    return rating || "n/a";
  }
  ...
  async function generateBatchPdf(reportData) {
    ...
    for (const item of reportData.successItems) {
      ...
      addLine(`Overall: ${item.overallDisplay || (item.overall === null ? "skip" : "n/a")}`, 10, 13);
      ...
      for (const dim of dims) {
        ...
        if (!row) continue;
        addWrapped(`${dim} (${ratingLabel(row)}): `, textOrNone(row.reasoning));
      }
